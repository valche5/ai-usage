...
cookie de session ; un client API peut envoyer `Authorization: Bearer <jeton>` — le mot de
passe web, ou `AI_USAGE_API_TOKEN` s'il est défini. L'API n'expose ni tokens OAuth,
...
| `AI_USAGE_API_TOKEN` | — | jeton Bearer optionnel dédié aux clients API |
